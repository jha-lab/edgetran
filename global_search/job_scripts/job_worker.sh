...
echo "python pretrain_model.py --output_dir ${model_dir} --steps ${steps} --learning_rate ${learning_rate} --trial_run" >> $job_file

echo "for i in {0..1}" >> $job_file
echo "do" >> $job_file
echo -e "\tssh \${node_list_arr[\${i}]} \"conda activate txf_design-space; \\" >> $job_file
echo -e "\t\tcd /scratch/gpfs/${id}/edge_txf/global_search/; \\" >> $job_file
